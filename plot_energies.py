...
def plot_energies(data, times, output_path='./'):
    ...
    ax1.plot(t, data['Nusselt_2'], label="Nu$_2$")
    ax1.plot(t, data['Nusselt_3'], label="Nu$_3$")
    ax1.plot(t, data['Nusselt_4'], label="Nu$_4$")
    ax1.plot(t, data['Nusselt_5'], label="Nu$_5$")
    ax1.legend()
    ax1.set_ylabel("Nu")
    ax2 = fig_Nu.add_subplot(2,1,2)
    ax2.semilogy(t, np.abs(data['Nusselt']), label="Nu")
    ax2.semilogy(t, np.abs(data['Nusselt_2']), label="Nu$_2$")
    ax2.semilogy(t, np.abs(data['Nusselt_3']), label="Nu$_3$")
    ax2.semilogy(t, np.abs(data['Nusselt_4']), label="Nu$_4$")
    ax2.semilogy(t, np.abs(data['Nusselt_5']), label="Nu$_5$")    
    ax2.legend()
    ax2.set_xlabel("time")
    ax2.set_ylabel("Nu")
    figs["Nu"] = fig_Nu
